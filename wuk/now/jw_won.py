...
SCORE_SIDE        = 2000.0  # 측방 레이어 방향 가중치 (방향 결정 주도)
...
SIDE_W_MAG_GAIN      = 1.0  # 측방 push 기반 w_mag: obstacle_push × MAX_W × gain (0이면 forward_urgency fallback)
...
def find_vw_layered(scan_points, heading_deg):
    ...
    5. w: max(forward_urgency, obstacle_push×MAX_W×gain) × direction + side_repulsion
    """
    # 1. 레이어 처리
    layer_results = []
    for layer in LAYERS:
    ...
    # 6. w 크기: 정면 urgency 기저 + 측방 push 기반 교체
    ...
    if DEBUG_FINAL:
        print(f"  [FINAL] v={v:.2f} w={w:+.2f} dir={'L' if direction > 0 else 'R'} "
              f"fwd_u={forward_urgency:.2f} obs_push={obstacle_push:.2f}")
