...
STOP_FWD_RANGE   = 125   # 전방 장애물까지의 거리가 125mm 이하가 되면 로봇의 전진(v)을 완전히 멈추는 구역입니다.
...
# ── 헤딩 방향 점수제 ──────────────────────────────────────────────────────────
HEADING_WEIGHT   = 0.3   # 원래 가야 할 방향(헤딩)으로 돌아가려는 성향의 가중치입니다. 1도당 여유공간 1도의 가치를 줍니다.
    ...
    while arduino.in_waiting > 0:         # 아두이노로부터 수신 버퍼에 쌓인 데이터가 있는 동안 계속 반복합니다.
        try:
            ...
            if line.startswith('H:'):     # 읽어온 문자열이 'H:'로 시작한다면 헤딩 데이터로 간주합니다.
                arduino_heading_deg = float(line[2:])  # 'H:' 뒷부분의 문자열을 실수(float)로 변환하여 헤딩 각도를 업데이트합니다.
...
def is_path_blocked(front_scan_points):
    ...
    for angle_norm, dist in front_scan_points:  # 전면 스캔 데이터 리스트를 순회합니다.
        if dist <= 0:                           # 거리가 0 이하인 에러 데이터는 무시합니다.
            continue
...
def find_escape_angle(all_scan_points):
    ...
    for angle_norm, dist in all_scan_points:    # 360도 모든 스캔 데이터를 순회합니다.
        if dist <= 0:                           # 무효 데이터(거리 0 이하)는 건너뜁니다.
            continue
...
def find_vw_command(scan_points, heading_deg):
    ...
    for angle_norm, dist in scan_points:        # 전방 스캔 데이터를 모두 확인합니다.
        if dist <= 0 or dist > DETECTION_RANGE: # 센서 에러이거나 너무 멀리 있는(1.5m 초과) 데이터는 위협이 아니므로 버립니다.
            continue
    ...
    if stop_points:                             # 충돌 직전(정지 상태)이라면 복잡한 빈 공간 계산보다 직관적인 생존이 우선입니다.
        ...
        for a in range(-SCAN_HALF_ANGLE, 0, ANGLE_STEP): # 스캔의 왼쪽 절반(음수 각도)을 뒤집어 봅니다.
            if scan_dict.get(a, DETECTION_RANGE + 1) >= ref_dist: # 장애물이 핵심 기준점 거리보다 멀리 떨어져 있으면 '빈 공간'으로 칩니다.
                left_clear += ANGLE_STEP        # 왼쪽 빈 공간 각도를 5도 증가시킵니다.
        for a in range(ANGLE_STEP, SCAN_HALF_ANGLE + 1, ANGLE_STEP): # 오른쪽 절반(양수 각도)도 동일하게 검사합니다.
