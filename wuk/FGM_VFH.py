...
MIN_VIABLE_WIDTH_MM = (ROBOT_HALF_WIDTH * 2) + 30  # (추가) 로봇 폭(220) + 여유(30) = 250mm 이상이어야 통과 가능
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def select_direction(left_width_mm, right_width_mm, heading_deg):
    # 1. 실제 빈 공간의 너비가 로봇이 통과할 수 있는 폭(250mm) 이상인지 평가합니다.
    left_ok  = left_width_mm  >= MIN_VIABLE_WIDTH_MM
    right_ok = right_width_mm >= MIN_VIABLE_WIDTH_MM

    # 2. 갇힘 방지: 한쪽이 물리적으로 너무 좁으면 강제로 넓은 쪽으로 피하게 강제합니다.
    if left_ok and not right_ok:                
        print(f"  [Dir] Right too narrow({right_width_mm:.0f}mm) → Force Left")
        return 1.0                              
    if right_ok and not left_ok:                
        print(f"  [Dir] Left too narrow({left_width_mm:.0f}mm) → Force Right")
        return -1.0                             
    if not left_ok and not right_ok:            
        print(f"  [Dir] Both narrow → Select {'Left' if left_width_mm >= right_width_mm else 'Right'}")
        return 1.0 if left_width_mm >= right_width_mm else -1.0 

    # 3. 양쪽 다 통과할 만큼 넓다면, 공간의 여유도(mm)와 헤딩 관성을 합산하여 점수를 매깁니다.
    # (mm 단위는 숫자가 크므로 헤딩 가중치의 밸런스를 맞추기 위해 10배를 곱해줍니다)
    left_score  = left_width_mm  + max(0.0, -heading_deg) * (HEADING_WEIGHT * 10)
    right_score = right_width_mm + max(0.0,  heading_deg) * (HEADING_WEIGHT * 10)

    bonus_side = "R" if heading_deg > 0 else "L" 
    bonus_val  = abs(heading_deg) * (HEADING_WEIGHT * 10) 
    
    print(f"  [DirScore] L={left_score:.0f}  R={right_score:.0f}  (Clear L={left_width_mm:.0f}mm R={right_width_mm:.0f}mm  HeadingBonus {bonus_side}+{bonus_val:.0f})")
...
def find_vw_command(scan_points, heading_deg):
    ...
    print(f"  [Clearance] L:{left_width_mm:.0f}mm  R:{right_width_mm:.0f}mm  Heading:{heading_deg:.1f}°")

    if avoidance_w_sign == 0.0: 
        # 이전에 피하던 방향이 없었다면 (새로운 회피 시작)
        avoidance_w_sign = select_direction(left_width_mm, right_width_mm, heading_deg) 
        print(f"  [DirDecide] Locked to {'Left' if avoidance_w_sign>0 else 'Right'}")
    else: 
        # 이미 피하고 있던 중이라면 (방향 관성 존재)
        committed_width = left_width_mm if avoidance_w_sign > 0 else right_width_mm 
        
        # 피하려고 돌고 있는 쪽의 '물리적 공간(mm)'이 로봇 폭보다 좁아졌다면 방향 전환 고려
        if committed_width < MIN_VIABLE_WIDTH_MM: 
