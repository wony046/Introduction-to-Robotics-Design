...
class Sim:
    def __init__(self):
        ...
        self.sim_time = 0.0          # 누적 시뮬 시간 (피버턴 주기/타이밍용)
...
def draw_map(screen, overlay, sim, font, font_s):
    ...
    # 경계 원 (새 로직: Mode1=도착지점 / Mode2=목표추정 중심, 가변 반경)
    if tg['bound'].checked:
        ctr = None
        if S._search_mode == 1 and S._last_arrival_x is not None:
            ctr = (-S._last_arrival_x, S._last_arrival_y, C_BOUND, 'arrival')
        elif S._search_mode == 2 and S._last_target_est_x is not None:
            ctr = (-S._last_target_est_x, S._last_target_est_y, (210, 140, 90), 'target_est')
        if ctr is not None:
            bcx, bcy, bcol, _ = ctr
            ox, oy = w2s(bcx, bcy, cam, zoom)
            pygame.draw.circle(screen, bcol, (int(ox), int(oy)),
                               int(S._current_boundary_radius * zoom), 1)
            pygame.draw.line(screen, bcol, (ox - 5, oy), (ox + 5, oy), 1)
            pygame.draw.line(screen, bcol, (ox, oy - 5), (ox, oy + 5), 1)
            # 반경 확장됐으면 기본 반경도 점선처럼 옅게
            if S._current_boundary_radius > S.BOUNDARY_RADIUS + 1:
                pygame.draw.circle(screen, (70, 70, 80), (int(ox), int(oy)),
                                   int(S.BOUNDARY_RADIUS * zoom), 1)
        # 피버턴 중이면 로봇 둘레 회전 인디케이터
        if S._pivot_active:
            pr = max(10, int((S.ROBOT_HALF_WIDTH + 70) * zoom))
            frac = min(1.0, S._pivot_total_rotated / 360.0)
            import math as _m
            n = max(2, int(40 * frac))
            arc = [(int(rpx + pr * _m.sin(2 * _m.pi * k / 40)),
                    int(rpy - pr * _m.cos(2 * _m.pi * k / 40))) for k in range(n + 1)]
            if len(arc) >= 2:
                pygame.draw.lines(screen, (250, 210, 90), False, arc, 2)
...
def draw_hud(screen, sim, font, font_s):
    ...
    # ── 새 로직: 탐색 모드 상태 ──
    mode_names = {0: 'NORMAL', 1: 'SEARCH-arrival', 2: 'SEARCH-lost'}
    sm = S._search_mode
    if sm != 0:
        extra = ''
        if S._pivot_active:
            extra = f'  PIVOT {S._pivot_total_rotated:.0f}/350deg'
        lines.append((f'searchMode={mode_names.get(sm, sm)}  '
                      f'R={S._current_boundary_radius:.0f}mm{extra}', (250, 210, 90)))
...
def make_default_scene(sim):
    """v2 데모(산개 배치, 장애물 없음): 도착 후 다음 색이 FOV 밖 → Mode1 피버턴 탐색 발동.
    RED 정면 → (피버턴) 좌측 YELLOW → (피버턴) 우상 BLUE.
    장애물은 사용자가 배치(회전 포함)해 회피+탐색을 함께 테스트."""
    sim.patches.append({'x': -100, 'y': -350, 'w': 200, 'h': 200, 'color': 'R'})  # center (0,-250) 정면
    sim.patches.append({'x': -700, 'y':    0, 'w': 200, 'h': 200, 'color': 'Y'})  # center (-600,100) 좌측
    sim.patches.append({'x':  150, 'y':  350, 'w': 200, 'h': 200, 'color': 'B'})  # center (250,450) 우상
...
def main():
    ...
    pygame.init()
    pygame.display.set_caption('Project 3 Driving Sim  v2  (Mode1/2 search logic)')
