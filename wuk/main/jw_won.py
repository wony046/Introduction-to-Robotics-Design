        ...
        if pts:
            ...
            # ── 상태 2: CLOSE → 오도메트리 위치 제어 ───────────────────────
                ...
                if dist_err < CLOSE_ARRIVE_MM:
                    ...
                    # 헤딩 오차가 클수록 v 감소 → 방향 수정하면서 곡선 접근
                    align_factor = max(0.0, 1.0 - abs(hdg_err) / CLOSE_ALIGN_DEG)
                    v_dist = min(CLOSE_SPEED_MAX, max(MIN_SPEED, 0.001 * dist_err))
                    v = MIN_SPEED + (v_dist - MIN_SPEED) * align_factor
                    ...
                    if DEBUG_CLOSE_POS:
                        print(f"[CLOSE] pos=({arduino_x_mm:.0f},{arduino_y_mm:.0f}) "
                              ...
                              f"align={align_factor:.2f} v={v:.2f} w={w:+.2f}")
