...
DEBUG_SEEK_LOST   = 1   # [SEEK_LOST] 색 소실 후 마지막 본 방향 헤딩 보정 추종
...
# ── 색지 도달 후 360° 피봇 (1.2초 dwell 정지가 끝난 뒤, 다음 색 탐색) ──
# 회전 중 색지를 발견하면 그 자리에 멈춰 짧게 확정한 뒤 추종을 시작(early-exit).
# 미발견이면 한 바퀴(360°)까지 회전 후 출발.
ARRIVE_PIVOT_W    = 1.0   # rad/s: 피봇 회전 속도 (부호=회전 방향)
PIVOT_CONFIRM_SEC = 0.15  # 회전 중 색지 발견 시 정지 확정 시간 (stale/노이즈 배제)
_pivot_pending    = False # 직전 색지 dwell 후 피봇 미수행 상태 (dwell 중 예약)
_pivot_accum      = 0.0   # 피봇 누적 회전량 (deg)
_pivot_prev_hdg   = None  # 피봇 델타 계산용 직전 헤딩
_pivot_confirm_start = None  # 회전 중 색지 후보 연속 감지 시작 시각 (None=미감지)
...
# ── SEEK 색지 소실 시 마지막 본 방향 추종 (오도메트리 헤딩 보정) ───────────────
# 색을 감지하는 동안 마지막으로 본 bearing 을 월드(글로벌) 방향으로 저장한다.
# 색이 소실되면(카메라 갱신 종료) 매 사이클 현재 오도메트리 헤딩으로 상대 bearing 을
# 재계산(헤딩 보정)하여, 로봇이 회전/이동해도 마지막으로 본 색지의 방향을 계속 추종한다.
# 색지 도달(DWELL) 시 None 으로 초기화되어 다음 색 탐색은 새 기억으로 시작.
_seek_last_global_bearing = None   # deg: 마지막 감지 시 색지의 글로벌 방향 (None=기억 없음)
...
def _get_boundary_correction():
    ...
    반환: (rel_bearing_deg, v_scale, exceeded)
        exceeded: 경계를 벗어났는가 (True 면 중심 복귀 인력 활성)"""
    if _boundary_center_x is None:
...
def _motor_controller(arduino):
    ...
    global _pivot_pending, _pivot_accum, _pivot_prev_hdg, _pivot_confirm_start
    global _boundary_center_x, _boundary_center_y
    global _seek_last_global_bearing
    last_cmd_str = ""
    while not _shutdown.is_set():
        ...
        if pts:
            ...
            if camera_tracker.is_done() or camera_tracker.is_dwelling():
                ...
                # 색지 도달 → 직전 색 추종 기억 폐기 (다음 색은 새 기억으로 탐색)
                _seek_last_global_bearing = None
                # 색지 dwell(1.2초 정지) 중이면 → 끝난 뒤 출발 직전 피봇 1회 예약.
                # (완주 DONE 상태의 dwell은 제외 — 더 이상 출발하지 않음)
                if camera_tracker.is_dwelling() and not camera_tracker.is_done():
                    _pivot_pending = True

            # ── 상태 P: 색지 dwell 직후 피봇 탐색 (회전 중 발견 시 멈추고 추종) ──
            elif _pivot_pending:
                bearing = camera_tracker.get_bearing()
                if bearing is not None:
                    # ── 색지 후보 발견 → 그 자리에 멈춰 짧게 확정 ──────────────
                    # (도달 직후 dwell 종료 프레임의 stale bearing·단발 노이즈 배제)
                    if _pivot_confirm_start is None:
                        _pivot_confirm_start = time.time()
                        if DEBUG_CLOSE_DONE:
                            print(f"[PIVOT] 색지 후보 {bearing:+.1f}° → 정지 확정 중")
                    v, w   = 0.0, 0.0     # 그 방향에서 멈춤
                    prev_w = 0.0
                    if time.time() - _pivot_confirm_start >= PIVOT_CONFIRM_SEC:
                        # 확정 → early-exit: 방향 기록 + 추종 시작
                        _pivot_pending       = False
                        _pivot_prev_hdg      = None
                        _pivot_accum         = 0.0
                        _pivot_confirm_start = None
                        # 마지막 본 방향 기록 (소실 시 헤딩 보정 추종에 사용)
                        _seek_last_global_bearing = normalize_angle(
                            arduino_heading_deg + bearing)
                        _boundary_center_x = None
                        _boundary_center_y = None
                        print(f"[PIVOT] 색지 확정 {bearing:+.1f}° → 피봇 중단, 추종 시작")
                        v, w = find_vw_command(pts, arduino_heading_deg,
                                               target_bearing=bearing)
                else:
                    # ── 미발견 → 확정 타이머 리셋, 360°까지 회전 ──────────────
                    _pivot_confirm_start = None
                    if _pivot_prev_hdg is None:
                        _pivot_prev_hdg = arduino_heading_deg
                        _pivot_accum    = 0.0
                        print("[PIVOT] 1.2초 정지 완료 → 색지 탐색 피봇 시작")
                    # 측정 헤딩 변화량 누적 (한 방향 회전이라 부호 일정)
                    _pivot_accum   += abs(normalize_angle(
                        arduino_heading_deg - _pivot_prev_hdg))
                    _pivot_prev_hdg = arduino_heading_deg

                    if _pivot_accum < 360.0:
                        v, w   = 0.0, ARRIVE_PIVOT_W   # 한 방향 제자리 회전
                        prev_w = w                     # 스무딩 관성 제거 (일정 속도 유지)
                        if DEBUG_CLOSE_DONE:
                            print(f"[PIVOT] 회전 중 ... {_pivot_accum:.0f}/360°")
                    else:
                        # 360° 완료(색지 미발견) → 예약 해제, 정상 SEEK 출발
                        _pivot_pending  = False
                        _pivot_prev_hdg = None
                        _pivot_accum    = 0.0
                        v, w   = 0.0, 0.0
                        prev_w = 0.0
                        print("[PIVOT] 360° 피봇 완료(미발견) → 출발")
            ...
            # _close_target_x가 이미 세팅돼 있으면 카메라 미감지 시에도 CLOSE 유지
                ...
                if dist_err < CLOSE_ARRIVE_MM:
                    ...
                    # signal_arrival() → 카메라 1.2초 dwell 시작 → 그 dwell이 끝난 뒤
                    #   상태 P에서 360° 피봇 후 출발 (피봇 예약은 상태 1에서 set)
                    v, w = 0.0, 0.0
                    prev_w = 0.0
            ...
            # ── 상태 3: SEEK → 카메라 bearing + 라이다 회피 ────────────────
                ...
                if bearing is not None:
                    ...
                    _seek_last_global_bearing = normalize_angle(arduino_heading_deg + bearing)
                    v, w = find_vw_command(pts, arduino_heading_deg, target_bearing=bearing)
                else:
                    # 색 미감지(카메라 갱신 종료): 마지막 본 색지의 글로벌 방향을 현재
                    # 헤딩 기준 상대 bearing 으로 변환(오도메트리 헤딩 보정)해 추종한다.
                    # 단, 경계(BOUNDARY_RADIUS) 초과 시엔 중심 복귀 인력이 우선(안전).
                    _set_boundary_center()
                    boundary_tb, v_scale, boundary_exceeded = _get_boundary_correction()
                    if boundary_exceeded:
                        seek_tb  = boundary_tb        # 경계 밖 → 중심 복귀 (안전 우선)
                        seek_src = 'BOUNDARY'
                    elif _seek_last_global_bearing is not None:
                        # 헤딩 보정: 월드 방향 - 현재 헤딩 → 로봇 상대 bearing
                        seek_tb  = normalize_angle(_seek_last_global_bearing
                                                   - arduino_heading_deg)
                        seek_src = 'LAST_BEARING'
                    else:
                        seek_tb  = 0.0                # 기억 없음(색 한번도 못 봄) → 직진
                        seek_src = 'STRAIGHT'
                    if DEBUG_SEEK_LOST:
                        gb = (f"{_seek_last_global_bearing:+.1f}"
                              if _seek_last_global_bearing is not None else "None")
                        print(f"[SEEK_LOST] {seek_src} tb={seek_tb:+.1f}° "
                              f"(global={gb}° hdg={arduino_heading_deg:+.1f}°) "
                              f"v_scale={v_scale:.2f}")
                    v, w = find_vw_command(pts, arduino_heading_deg, target_bearing=seek_tb)
                    v *= v_scale
