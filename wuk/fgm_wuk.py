...
MAX_RANGE        = 2500  # mm: 이 거리보다 멀면 빈 공간(Gap)으로 취급
SAFE_DIST        = 600   # mm: 최소 통과 보장 거리 임계값
MIN_GAP_DEPTH    = 1200  # mm: 갇힘(ㄱ자) 방지. Gap 내부 최대 거리가 이보다 길어야 함
GAP_MARGIN_DEG   = 10    # deg: 갭의 가장자리를 탈 때 추가로 띄우는 안전 각도

# ── 속도 및 제어 파라미터 ─────────────────────────────────────────────────────
FORWARD_SPEED    = 0.35  # m/s: 기본 직진 속도
MIN_SPEED        = 0.10  # m/s: 회전 시 최소 속도
