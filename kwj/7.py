...
def _motor_controller(arduino):
    ...
    while not _shutdown.is_set():
        ...
        # ★ 추가 1: 카메라가 "색지 완료" 신호를 주면 오도메트리 리셋 (나선 탐색 중심점 갱신)
        if camera_tracker.check_mission_changed():
            arduino.write(b"R\n")
            arduino_x_mm, arduino_y_mm, arduino_heading_deg = 0.0, 0.0, 0.0
