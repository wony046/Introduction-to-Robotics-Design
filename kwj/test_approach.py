        ...
        if contours:
            ...
            if area > 500:
                ...
                if M['m00'] != 0:
                    ...
                    if depression > 1.0:
                        dist_mm = CAM_HEIGHT_MM / math.tan(math.radians(depression))
                        dist_mm = max(dist_mm, 50.0)
        ...
        if state == "SEEK":
            if cx is not None:
                if dist_mm < CLOSE_ENTER_MM:
                    print(f"\n[SEEK -> OBSERVE] 목표물 접근! (거리: {dist_mm:.0f}mm < {CLOSE_ENTER_MM}mm)")
                    state = "OBSERVE"
                    ...
                    print(f"\r[SEEK] 직진 중... 거리: {dist_mm:4.0f}mm | 조향각: {bearing:+5.1f}°", end="")
            else:
                # 못 찾음: 제자리 정지 (테스트용)
            ...
            if elapsed >= CLOSE_OBSERVE_SEC:
                ...
                print(f"\n[OBSERVE -> BLIND] 절대 목표 좌표 확정: ({target_x:.0f}, {target_y:.0f})")
            ...
            if dist_err < CLOSE_ARRIVE_MM:
                ...
                print(f"\r[BLIND] 눈 감고 주행 중... 진행률: {progress:5.1f}% | 남은 거리: {dist_err:3.0f}mm", end="")
