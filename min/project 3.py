...
# 2) 피벗 중 너무 가까운 장애물: 제자리 회전 자리에 장애물이 SEARCH_PIVOT_CLEAR_MM
#    이내로 들어오면 그 지점을 즉시 건너뛰고 다음 지점으로.
# 3) 모든 웨이포인트를 다 포기하면: 중심으로 돌아가 처음부터 반복.
SEARCH_WP_TIMEOUT_SEC  = 10.0   # sec: 한 웨이포인트 최대 체류 시간 (초과 시 포기)
SEARCH_PIVOT_CLEAR_MM  = 200.0  # mm: 피벗 중 이 거리 이내 장애물 있으면 지점 건너뜀
SEARCH_PIVOT_CLEAR_CONE = 70.0  # deg: 너무 가까운 장애물 검사 콘 (정면 ± 이 각도)
...
def _pivot_obstacle_too_close(scan_points):
    """피벗 자리 정면 ± SEARCH_PIVOT_CLEAR_CONE 콘 안에 SEARCH_PIVOT_CLEAR_MM
    이내 장애물이 있으면 True (그 지점 건너뛰기 트리거)."""
    for angle_norm, dist in scan_points:
...
def _search_compute_command(scan_points):
    ...
    안전장치 (무한 정체 방지):
      ...
      ② 피벗 중 너무 가까운 장애물: 즉시 그 지점 건너뜀→다음.
    ...
    # ── phase: 제자리 피벗 스캔 ──────────────────────────────────────────
    # 안전장치 ②: 피벗 자리에 장애물이 너무 가까우면 그 지점 건너뜀
    if _pivot_obstacle_too_close(scan_points):
        _search_advance_wp(f"피벗 장애물 근접(<{SEARCH_PIVOT_CLEAR_MM:.0f}mm) 건너뜀",
                           completed=False)
        return 0.0, 0.0
