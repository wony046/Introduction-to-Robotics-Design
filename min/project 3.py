...
# 2) 피벗 중 근접 장애물(회피 최우선): 회전 스캔보다 '회피'를 무조건 우선한다.
#    전방 콘 안 SEARCH_PIVOT_CLEAR_MM 이내에 장애물이 잡히거나 STOP존이 활성이면,
#    제자리 회전을 멈추고 회피 시스템(find_vw_command: STOP 탈출 + 레이어 회피)에
#    제어를 넘겨 먼저 빠져나간 뒤, 장애물이 사라지면 다음 사이클에 피벗을 이어간다.
# 3) 모든 웨이포인트를 다 포기하면: 중심으로 돌아가 처음부터 반복.
SEARCH_WP_TIMEOUT_SEC  = 10.0   # sec: 한 웨이포인트 최대 체류 시간 (초과 시 포기)
SEARCH_PIVOT_CLEAR_MM  = 200.0  # mm: 피벗 중 이 거리 이내 장애물 발견 시 회피 우선
SEARCH_PIVOT_CLEAR_CONE = 70.0  # deg: 근접 장애물 검사 콘 (정면 ± 이 각도)
...
def _nearest_pivot_obstacle(scan_points):
    """피벗 자리 정면 ± SEARCH_PIVOT_CLEAR_CONE 콘 안, SEARCH_PIVOT_CLEAR_MM 이내
    최근접 장애물의 (상대각도°, 거리mm). 없으면 None."""
    nearest = None
    for angle_norm, dist in scan_points:
        if dist < LIDAR_MIN_VALID:
            continue
        if abs(normalize_angle(angle_norm)) <= SEARCH_PIVOT_CLEAR_CONE \
           and dist < SEARCH_PIVOT_CLEAR_MM:
            if nearest is None or dist < nearest[1]:
                nearest = (angle_norm, dist)
    return nearest
...
def _search_compute_command(scan_points):
    ...
    # ── phase: 제자리 피벗 스캔 ──────────────────────────────────────────
    # ★ 회피 최우선: 제자리 회전(스캔) 중 전방 콘 안 근접 장애물이 잡히거나 STOP존이
    #   활성이면, 회전을 멈추고 곧바로 회피 시스템(find_vw_command: STOP 탈출 +
    #   레이어 회피)에 제어를 넘긴다. 장애물을 벗어나면 다음 사이클에 피벗을 이어간다.
    nearest = _nearest_pivot_obstacle(scan_points)
    if nearest is not None or detect_stop_zone(scan_points):
        ...
        if DEBUG_SEARCH:
            near_str = (f"near={nearest[1]:.0f}mm@{nearest[0]:+.0f}°"
                        if nearest is not None else "STOP존")
            print(f"  [SEARCH] pivot@wp{_search_corner_idx} 회피 우선({near_str}) "
                  f"→ v={v:.2f} w={w:+.2f}")
        return v, w

    # 누적 회전량 갱신 (장애물 없이 통제된 제자리 스캔이 진행된 만큼)
