...
# 4) 피벗 위치 위험 판단: 피벗 단계에서 장애물 회피(이격 후진/STOP/위치 복귀)만 이
#    시간만큼 지속돼 제자리 피벗을 전혀 진행하지 못하면, 그 지점은 '위험'으로 보고
#    피벗을 완료한 것으로 처리해(completed) 다음 웨이포인트로 넘어간다.
#    (WP 타임아웃보다 짧게 둬, 위험 지점은 더 빨리 포기)
SEARCH_PIVOT_DANGER_SEC = 3.0   # sec: 회피만 지속되면 피벗 생략·다음 지점 이동
...
_search_pivot_block_start = None # 피벗 위치에서 회피만 반복(피벗 진행 불가) 시작 시각
                                 #   (위험 판단 타이머용; 피벗 진행/웨이포인트 전환 시 None)
...
def _search_reset():
    ...
    global _search_pivot_block_start
...
def _search_advance_wp(reason, completed):
    ...
    global _search_pivot_accum, _search_pivot_start_hdg, _search_prev_hdg
    global _search_abandoned, _search_pivot_block_start

    _search_pivot_block_start = None   # 새 웨이포인트 → 위험 판단 타이머 리셋
...
def _search_compute_command(scan_points):
    ...
    global _search_pivot_start_hdg, _search_pivot_accum, _search_prev_hdg
    global _search_wp_start_time, _search_pivot_block_start
    ...
    #
    # 단, ①②(회피/복귀)만 SEARCH_PIVOT_DANGER_SEC 넘게 지속돼 ③ 피벗을 전혀 진행하지
    # 못하면 → 이 지점은 장애물로 '위험'하다고 보고, 피벗을 완료한 것으로 처리해
    # 다음 웨이포인트로 넘어간다. (회피 상태를 한 번만 계산해 위험 타이머에 사용)
    ...
    if blocked:
        if _search_pivot_block_start is None:
            _search_pivot_block_start = now
        elif (now - _search_pivot_block_start) >= SEARCH_PIVOT_DANGER_SEC:
            # 위험 판단: 피벗을 못 한 채 회피만 지속 → 피벗 완료 처리(completed) 후 다음 지점
            if DEBUG_SEARCH:
                print(f"  [SEARCH] pivot@wp{_search_corner_idx} 위치 위험 "
                      f"(회피 {SEARCH_PIVOT_DANGER_SEC:.0f}s 지속) → 피벗 생략, 다음 지점")
            _search_advance_wp("피벗 위치 위험(장애물) — 피벗 완료 처리", completed=True)
            return 0.0, 0.0
    else:
        _search_pivot_block_start = None   # 피벗 진행 가능 → 위험 타이머 해제
