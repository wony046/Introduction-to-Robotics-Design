# -*- coding: utf-8 -*-
"""
 통합 단일 파일 : avoidance_lidar + camera_tracker1 + 제어
   실행:  python3 robot.py   (출발지 정지 상태에서 실행, 이후 입력 불가)
   원본 3파일을 한 파일로 합침. 별도 import 불필요.
"""
import serial
import time
import math
...
# ===========================================================================
# camera_tracker  (색 검출 · bearing · 도착/1초정지/완주 — 미션 소유)
#   _shutdown 은 위 엔진의 것을 공유한다.
# ===========================================================================
import cv2
import math
import time
...
COLOR_RANGES = {
    'RED': [
        ((118, 104, 136), (179, 255, 255)),   # 실측값
    ],
    'YELLOW': [
        ((9, 90, 64), (41, 194, 255)),   # 실측값
    ],
    'BLUE': [
        ((107, 93, 109), (127, 182, 180)),   # 실측값
    ],
}
...
# ═══════════════════════════════════════════════════════════════════════════════
# 통합 제어  (avoidance 엔진 + camera_tracker 결합)
# ───────────────────────────────────────────────────────────────────────────────
#  · 카메라(camera_tracker)가 미션순서·도착·1초정지(dwell)·완주를 소유:
#      get_bearing() / is_dwelling() / is_done() / get_state()
#  · 제어부는 주행만 담당:
#      is_done()/is_dwelling() → 정지,  bearing 있음 → TRACK,
#      bearing None → SPIN(제자리 360°) → WANDER(빈공간 전진), 허허벌판 → RETURN
#  · 도착 정지 시 스무딩 우회 + 엔진 STOP 상태 리셋(잔여 회전/피봇 간섭 차단)
#  · 아두이노 ±90° 헤딩가드는 ESC 주기 재전송으로 상시 해제(자유 360° 회전)
# ═══════════════════════════════════════════════════════════════════════════════

# 엔진 디버그 출력 OFF (콘솔 정리)
DEBUG_LAYERS = DEBUG_STOP = DEBUG_DIR = DEBUG_FINAL = DEBUG_SIDE = DEBUG_VIRTUAL = False

# ── 제어 파라미터 (★ = 현장 캘리브레이션) ─────────────────────────────────────
GOAL_W_GAIN       = 1.6
BEARING_TO_W_SIGN = -1.0     # +bearing(목표 오른쪽)→우회전(w<0). 카메라 좌우 반전 시 +1.0
BEARING_DEADBAND  = 6.0      # deg
TRACK_SPEED       = 0.30     # 추적 전진 상한(엔진 v_a 와 min)

SPIN_W            = 0.8      # rad/s 제자리 스캔
SPIN_FULL_TURN    = 2*math.pi*1.05   # 한 바퀴(+5%) 누적 → WANDER
WANDER_SPEED      = 0.32     # 배회 전진 상한

OPEN_NEAR_DIST    = 1200     # mm: 전 방향 최근접이 이보다 멀면 '주변에 물체 없음'
OPEN_FIELD_DIST   = 1500     # mm: 전방 180° 평균이 이보다 멀면 '허허벌판'
RETURN_SPIN_W     = 0.9      # rad/s 복귀 회전
RETURN_FACE_TOL   = 25       # deg
RETURN_SPEED      = 0.30

MATCH_TIME_LIMIT_S = 178.0   # 3분 - 마진
MAIN_LOOP_HZ       = 20
ESC_KEEPALIVE_S    = 12.0    # 아두이노 ESCAPE_TIMEOUT(18s)보다 짧게
...
def is_open_field(scan):
    """허허벌판(이탈) 판정: 주변에 가까운 물체가 없고 전방 평균이 멀다."""
...
def arena_direction(scan):
    """가장 가까운 물체 방향(deg, +오른쪽)·거리(mm) = 경기장 쪽. 없으면 (None,None)."""
...
class Driver:
    """주행 상태머신 (도착/정지/미션은 카메라가 소유)."""
    TRACK, SPIN, WANDER, RETURN = "TRACK", "SPIN", "WANDER", "RETURN"
    ...
    def step(self, scan, dt):
        b = get_bearing()
        # A. TRACK : bearing 정상이면 무조건 추적 우선
        if b is not None:
            self.mode = self.TRACK
            self._say(f"TRACK bearing={b:+.1f}° [{get_state()}]")
            return self._track(b, scan)

        if self.mode == self.TRACK:
            self._enter_spin()

        # 허허벌판(이탈) → RETURN (목표 안 보일 때만)
        if is_open_field(scan):
            if self.mode != self.RETURN:
                self.mode = self.RETURN
                self._say("허허벌판 감지(이탈) → RETURN(경기장 복귀)")
            return self._return(scan)
        elif self.mode == self.RETURN:
            self._enter_spin()
            self._say("경기장 복귀 완료 → SPIN")
        ...
        if self.mode == self.SPIN:
            ...
            if self._spin_accum >= SPIN_FULL_TURN:
                ...
                self._say(f"360° 스캔 실패 → WANDER [{get_state()}]")
                return self._wander(scan)
            self._say(f"제자리 스캔(SPIN) [{get_state()}]")
            return 0.0, self._spin_dir * SPIN_W

        # C. WANDER : FGM 회피로 빈 공간 전진
        if self.mode == self.WANDER:
            self._say(f"배회 탐색(WANDER) [{get_state()}]")
            return self._wander(scan)

        self._enter_spin()
        return 0.0, self._spin_dir * SPIN_W

    def _track(self, b, scan):
        v_a, w_a  = find_vw_command(scan, 0.0)
...
def _control_loop(arduino, driver, t_start):
    ...
    while not _shutdown.is_set():
        ...
        read_arduino(arduino)
        with _scan_lock:
            scan = [(a, d) for a, d in _latest_scan if d > 0]

        now = time.time(); dt = now - t_prev; t_prev = now

        # 헤딩가드 상시 해제(자유 360° 회전) — ESC 주기 재전송
        if now - last_esc >= ESC_KEEPALIVE_S:
            arduino.write(b"ESC\n")
            last_esc = now

        # 카메라가 도착/정지/완주 소유: dwell·done 이면 즉시·완전 정지
        if is_done() or is_dwelling():
            if not frozen:                       # 진입 1회: 엔진 STOP 상태 리셋
                try:
                    _stop_reset()
                except Exception:
                    pass
                frozen = True
            prev_w = 0.0                          # 스무딩 우회(잔여 회전 차단)
            arduino.write(b"0.00 0.00\n")
            tag = "DONE" if is_done() else "DWELL"
        else:
            frozen = False
            v, w = driver.step(scan, dt)
            w = W_SMOOTH * w + (1.0 - W_SMOOTH) * prev_w   # 엔진과 동일 스무딩
            prev_w = w
            arduino.write(f"{v:.2f} {w:.2f}\n".encode())
            tag = driver.mode

        if tag != last_log:
            print(f"[CTRL] {tag:6s}  state={get_state()}")
            last_log = tag
...
def main():
    print("=" * 70)
    print(" 통합 색상영역 추적 로봇  RED → YELLOW → BLUE")
    print("  avoidance(LiDAR 회피) + camera_tracker(색·도착) + 제어(TRACK/SPIN/WANDER/RETURN)")
    print(f"  도달정지 {ARRIVE_HOLD_SEC}s | 시간제한 {MATCH_TIME_LIMIT_S}s | ESC keepalive {ESC_KEEPALIVE_S}s")
    print("=" * 70)

    lidar   = serial.Serial(LIDAR_PORT,   BAUDRATE_LIDAR,   timeout=1)
    arduino = serial.Serial(ARDUINO_PORT, BAUDRATE_ARDUINO, timeout=1)
    time.sleep(3)                                  # 아두이노 부팅/EEPROM 로드 대기
    arduino.write(b"ESC\n"); time.sleep(0.1)       # heading 리셋 + 헤딩가드 해제(R은 Serial1에서 무시)
    lidar.write(bytes([0xA5, 0x40])); time.sleep(1)
    lidar.write(bytes([0xA5, 0x20])); lidar.read(7)

    _shutdown.clear()
    start()                                        # 카메라 스레드 (camera_tracker)
    driver  = Driver()
    t_start = time.time()

    t_lidar = threading.Thread(target=_lidar_reader, args=(lidar,), daemon=True, name="lidar")
    t_ctrl  = threading.Thread(target=_control_loop, args=(arduino, driver, t_start),
