...
HFOV_DEG       = 38.6     # 실측값
CAM_HEIGHT_MM  = 592.0    # ★ 실측 필요 (바닥~카메라 수직 높이 mm)
...
# ── 색 기준값: LAB 색공간 (OpenCV 8bit, a/b는 128=중립) ─────────────────────
#   각 색의 기준 (a, b)에서 a-b 평면상 거리가 TOL 이내이고 L >= L_MIN 이면 그 색으로 판정.
REF_AB = {              # (a, b) 측정 기준값
    'RED':    (166, 132),
    'YELLOW': (119, 191),
    'BLUE':   ( 99,  88),
}
TOL   = {'RED': 35, 'YELLOW': 35, 'BLUE': 35}   # a-b 평면 허용 반경
L_MIN = 30                                       # 이 밝기 미만은 제외(그림자/검정)
...
# ─────────────────────────────────────────────────────────────────────────────
def detect(frame):
    lab = cv2.cvtColor(frame, cv2.COLOR_BGR2LAB).astype(np.int32)
    L = lab[:, :, 0]
    a = lab[:, :, 1]
    b = lab[:, :, 2]
    ref_a, ref_b = REF_AB[TARGET_COLOR]
    tol = TOL[TARGET_COLOR]
    dist2 = (a - ref_a) ** 2 + (b - ref_b) ** 2          # a-b 평면 거리²
    mask  = ((dist2 <= tol * tol) & (L >= L_MIN)).astype(np.uint8) * 255
...
def main():
    ...
    while True:
        ...
        # 화면 중앙 30x30 패치의 평균 L,a,b (REF_AB/TOL 튜닝·검증용)
        _lab = cv2.cvtColor(last_frame, cv2.COLOR_BGR2LAB)
        _h, _w = _lab.shape[:2]
        cL, ca, cb = _lab[_h//2-15:_h//2+15, _w//2-15:_w//2+15] \
        ...
        if centroid is not None:
            ...
            print(f"\r[LIVE] cy={cy:4d}  dv={delta_v:+5.1f}°  "
                  ...
                  f"center Lab=({cL:3d},{ca:3d},{cb:3d})    ",
                  end='', flush=True)
        else:
            print(f"\r[LIVE] 미감지  center Lab=({cL:3d},{ca:3d},{cb:3d})        ",
                  end='', flush=True)
