...
MID_EMPTY_LIMIT = 0.10       # 중단 10% 미만 
TOP_EMPTY_LIMIT = 0.10       # 상단 10% 미만
...
SCORE_COLOR_TARGET = 5000.0  
SCORE_EXPLORE_BIAS = 600.0   
SCORE_ALIGN = 80.0           
...
current_color_idx = 0        
mission_phase = 0            
# Phase 0: 주행, 1: 1초 전진, 2: 1초 정지, 3: 고속 회전탐색, 99: 완료
arrive_time = 0.0
...
def find_vw_command(scan_points, heading_deg):
    ...
    if mission_phase > 0:
        ...
        # 🚨 [Phase 1] 도착 판정 직후: 1초간 추가 전진 (바퀴를 종이 위로)
        if mission_phase == 1:
            # 전진 도중 눈앞에 벽이 나타나면 박치기 방지를 위해 즉시 전진 취소 및 정지
            ...
            if elapsed < 1.0:
                return FORWARD_SPEED, 0.0 # 직진
            else:
                mission_phase = 2
                arrive_time = time.time()
                return 0.0, 0.0
                
        # 🚨 [Phase 2] 도착 인증 대기: 1초간 완전 정지 
        ...
        # 🚨 [Phase 3] 다음 타겟 스캔: 제자리 고속 회전
            ...
            # 회전 속도가 3배(2.4) 빨라졌으므로, 타임아웃을 2.5초로 줄임 (약 1바퀴 도는 시간)
            elif elapsed > 2.5:
    ...
    if visible and (bot >= ARRIVE_FILL_RATIO) and (mid < MID_EMPTY_LIMIT) and (top < TOP_EMPTY_LIMIT):
        print(f"\n[MISSION] TRIGGERED at {MISSION_COLORS[current_color_idx % len(MISSION_COLORS)]}!")
        current_color_idx += 1
        
        if current_color_idx >= len(MISSION_COLORS):
            print("[MISSION] ALL MISSIONS CLEARED!!!")
            mission_phase = 1 # 마지막 미션도 1초 전진은 수행
            # 단, 다음 루프에서 처리되도록 current_color_idx는 오버플로우로 둠
        else:
            print(f"[MISSION] Entering Phase 1 (1-sec forward drive).")
            mission_phase = 1 
            
        arrive_time = time.time()
        return FORWARD_SPEED, 0.0 # 멈추지 않고 일단 직진
    ...
    while not _shutdown.is_set():
        ...
        if contours:
            ...
            if cv2.contourArea(c) > MIN_CONTOUR_AREA:
                ...
                # 🚨 [핵심] 가로/세로 비율 기반 장애물 필터링
                aspect_ratio = box_w / float(box_h)
                # 세로로 긴 물체 (원통 장애물) 무시! (가로가 세로보다 0.7배 이하면 무시)
                if aspect_ratio < 0.7:
                    # found=False인 상태로 남겨 카메라가 인식 안 한 것처럼 처리
                    pass 
                else:
                    found = True
                    cx = x + box_w // 2
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def main():
    print("=== Robot Navigation + Vision Parking V2 ===")
