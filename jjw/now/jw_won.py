...
PIVOT_INTERVAL_SEC = 5.0    # sec: (구) 시간 기반 재피버턴 주기 — 현재 미사용(갭 기반 트리거로 대체)
MODE2_TIMEOUT_SEC  = 60.0   # sec: Mode2에서 이 시간 초과 시 Mode1으로 복귀
            ...
            # ── 상태 3: SEEK → 카메라 bearing + 라이다 회피 (+ Mode1/Mode2 탐색) ──
                ...
                if bearing is not None:
                    ...
                    if _pivot_active:
                        ...
                        # 갭 기반 재피버턴: 새 갭 등장 / 시야 변화 시에만 (시간 무관)
                        # 개활지는 find_passable_gap_for_pivot 필터에서 이미 배제됨
                        pivot_trigger = (None if detect_stop_zone(pts)
                                         else _should_pivot_for_gap(pts))
                        if (pivot_trigger is not None and
                                is_pivot_clearance_ok(pts, PIVOT_CLEAR_RADIUS)):
                            pivot_gap, gx, gy = pivot_trigger
                            _pivot_active        = True
                            _pivot_prev_hdg      = arduino_heading_deg
                            _pivot_total_rotated = 0.0
