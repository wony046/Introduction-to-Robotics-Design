...
GOAL_BIAS_WEIGHT  = 8.0           # mm/deg: 분기③(갭 없는 회피)에서 목표 방향 쪽
                                  #   score 가산 → 장애물 사이에서도 목표/경계 중심으로
                                  #   복귀 편향. head(5.0)보다 크게 두어 글로벌 0°
                                  #   헤딩 편향을 이김. 복귀 약하면 ↑, 장애물에 들이대면 ↓
...
def find_vw_layered(scan_points, heading_deg, target_bearing=0.0):
    ...
    if not blocked:
        ...
        # 목표 방향 bias: 갭이 없어 회피만 할 때도 목표/경계 중심 쪽으로 편향.
        # target_bearing 부호 = 카메라 bearing과 동일(+ = 목표가 물리적 우측).
        # 우측 목표 → 우회전(score_R), 좌측 목표 → 좌회전(score_L).
        term_goal_L = max(0.0, -target_bearing) * GOAL_BIAS_WEIGHT
        term_goal_R = max(0.0,  target_bearing) * GOAL_BIAS_WEIGHT
