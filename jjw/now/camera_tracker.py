...
# CLAHE 전처리 후 적용. REF_AB(a,b 중심) ± TOL, L >= L_MIN 으로 lo/hi 자동 생성 (재캘리).
#   범위: lo=(L_MIN[c], a-tol, b-tol), hi=(255, a+tol, b+tol)
#   L_MIN은 색상별 값 — 어두운 노이즈 컷 강도가 색마다 다름.
REF_AB = {
    'RED':    (210, 145),
    'YELLOW': ( 97, 182),
    'BLUE':   (116,  73),
}
TOL   = {'RED': 43, 'YELLOW': 40, 'BLUE': 34}
L_MIN = {'RED': 73, 'YELLOW': 109, 'BLUE': 59}

COLOR_RANGES = {}
for color in ['RED', 'YELLOW', 'BLUE']:
    a, b = REF_AB[color]
    tol  = TOL[color]
    lower = (L_MIN[color], max(0, a - tol), max(0, b - tol))
    upper = (255,          min(255, a + tol), min(255, b + tol))
...
def _camera_loop():
    ...
    # 자동 화이트밸런스 OFF — 프레임마다 색온도가 변하면 LAB 캘리(REF_AB)가 어긋난다.
    # 색 일관성을 위해 AUTO_WB를 끈다. (백엔드가 미지원이면 무시됨)
