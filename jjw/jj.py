...
def normalize_angle(angle):
    """
    RPLIDAR 원시 각도(0~360°, CCW 기준) →  -180~+180° (CW 기준, 양수=오른쪽)

    RPLIDAR C1은 위에서 봤을 때 반시계(CCW) 방향으로 스캔하므로:
      원시  90° = 실제 왼쪽  →  정규화 후 -90°
      원시 270° = 실제 오른쪽 → 정규화 후 +90°
    부호 반전으로 코드 전체의 "양수=오른쪽" 규칙과 일치시킴
    """
    a = angle - 360 if angle > 180 else angle
    return -a   # CCW → CW 변환
...
def check_rotation_blocked(w_sign, scan_points):
    ...
    if w_sign > 0:   # 왼쪽 회전 → 왼쪽 근거리(음수 각도) 확인
        check = range(-ANGLE_STEP, -61, -ANGLE_STEP)
    else:            # 오른쪽 회전 → 오른쪽 근거리(양수 각도) 확인
        check = range(ANGLE_STEP, 61, ANGLE_STEP)
...
def select_direction(left_clear, right_clear, heading_deg):
    ...
    left_clear  = 음수 각도 구간(왼쪽)의 열린 각도
    right_clear = 양수 각도 구간(오른쪽)의 열린 각도
    heading_deg > 0 = 왼쪽으로 틀어진 상태 → 오른쪽 보너스
    ...
    # 양쪽 모두 통과 가능 → 점수제
    # heading > 0 (왼쪽으로 틀어짐) → 오른쪽 방향(right_score)에 보너스
