...
MIN_SPEED        = 0.15  # 로봇이 장애물에 근접해 감속하더라도, 완전히 멈추지 않고 유지할 최소 직진 속도(0.07 m/s)입니다.
...
STOP_HORIZ_RANGE = 115   # 전진을 멈추는 구역의 좌우 폭(110mm)을 지정합니다. (로봇 반폭과 동일)
...
SIDE_CHECK_ANGLE = 90    # 측면 안전을 확인할 때 기준이 되는 라이다 스캔 각도 범위(±60도)입니다.
...
# [알고리즘 3] 열린공간 너비계산
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def get_max_open_width(scan_dict, angles, min_dist):
    """
    주어진 각도 범위(angles) 내에서 min_dist 이상 뚫려있는 
    가장 넓은 연속 공간의 물리적 너비(mm)를 제2코사인법칙으로 계산해 반환합니다.
    """
    max_w = 0.0
    in_open = False
    l_angle = None
    ...
    for idx, a in enumerate(angles_list):
        d = scan_dict.get(a, 0)
        is_open = (d >= min_dist)
        ...
        if not in_open:
            if is_open:
                in_open = True
                # 열린 구간 시작점 (직전 닫혀있던 장애물을 왼쪽 경계로 삼음)
                if idx > 0:
                    l_angle = angles_list[idx - 1]
                    l_dist = scan_dict.get(l_angle, min_dist) or min_dist
                else:
            ...
            if not is_open:
                in_open = False
                # 열린 구간 종료점 (현재 닫힌 장애물을 오른쪽 경계로 삼음)
                r_angle = a
                r_dist = d or min_dist
                
                # 제2코사인법칙으로 실제 너비 계산
                theta = math.radians(abs(r_angle - l_angle))
                w = math.sqrt(l_dist**2 + r_dist**2 - 2 * l_dist * r_dist * math.cos(theta))
                if w > max_w:
                    max_w = w
                    
    # 루프가 끝났는데 오른쪽 끝까지 계속 열려있을 경우의 마무리가 필요함
    if in_open:
        r_angle = angles_list[-1] + ANGLE_STEP
        r_dist = scan_dict.get(angles_list[-1], min_dist) or min_dist
        theta = math.radians(abs(r_angle - l_angle))
        w = math.sqrt(l_dist**2 + r_dist**2 - 2 * l_dist * r_dist * math.cos(theta))
        if w > max_w:
            max_w = w
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# [알고리즘 4] 회피 방향 가중치 점수 평가
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
def select_direction(left_width, right_width, heading_deg):
    # 로봇 전체 폭 + 안전 마진 (예: 110 * 2 + 10 = 230mm)
    MIN_VIABLE_WIDTH = (ROBOT_HALF_WIDTH * 2) + SAFETY_MARGIN
    
    left_ok  = left_width  >= MIN_VIABLE_WIDTH
    right_ok = right_width >= MIN_VIABLE_WIDTH

    if left_ok and not right_ok:                
        print(f"  [Dir] Right blocked({right_width:.0f}mm) → Force Left")
        return 1.0                              
    if right_ok and not left_ok:                
        print(f"  [Dir] Left blocked({left_width:.0f}mm) → Force Right")
        return -1.0                             
    if not left_ok and not right_ok:            
        print(f"  [Dir] Both narrow → Select {'Left' if left_width >= right_width else 'Right'}")
        return 1.0 if left_width >= right_width else -1.0 

    # 점수제 계산 (단위: mm)
    # 기존 HEADING_WEIGHT(1.0)는 각도 기반이었습니다. 
    # 이제 여유 공간이 mm 단위(보통 수백 mm)이므로, 헤딩 가중치도 스케일을 키워줍니다.
    # 예: 헤딩 1도당 15mm의 공간 가치를 지닌다고 설정
    HEADING_WEIGHT_MM = HEADING_WEIGHT * 15.0 
    ...
    print(f"  [DirScore] L={left_score:.0f}  R={right_score:.0f}  (Width L={left_width:.0f}mm R={right_width:.0f}mm  HeadingBonus {bonus_side}+{bonus_val:.0f})")

    return 1.0 if left_score >= right_score else -1.0


# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# [알고리즘 5] 모터 제어 명령(v, w) 산출의 코어 로직
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
# (기존 코드)
    ...
    # [수정된 코드 3] 방향(부호) 결정을 위한 좌우 물리적 너비(mm) 계산
    left_angles = range(-SCAN_HALF_ANGLE, 0, ANGLE_STEP)
    right_angles = range(ANGLE_STEP, SCAN_HALF_ANGLE + 1, ANGLE_STEP)
    
    left_clear_width = get_max_open_width(scan_dict, left_angles, ref_dist)
    right_clear_width = get_max_open_width(scan_dict, right_angles, ref_dist)

    if stop_points:
        print(f"  [StopZone] Active - Evaluating best clearance")
    
    # 단위가 각도(°)에서 밀리미터(mm)로 바뀌었음을 출력에 반영
    print(f"  [Clearance] L:{left_clear_width:.0f}mm  R:{right_clear_width:.0f}mm  Heading:{heading_deg:.1f}°")
    ...
    if avoidance_w_sign == 0.0: 
        ...
        # 기존 25도 대신 로봇 폭을 기준으로 재평가
        MIN_VIABLE_WIDTH = (ROBOT_HALF_WIDTH * 2) + SAFETY_MARGIN 
        if committed_clear < MIN_VIABLE_WIDTH: 
            old = avoidance_w_sign 
            avoidance_w_sign = select_direction(left_clear_width, right_clear_width, heading_deg) 
            if avoidance_w_sign != old: 
                print(f"  [DirSwitch] Blocked({committed_clear:.0f}mm) → {'Left' if avoidance_w_sign>0 else 'Right'}")
