...
ESCAPE_W             = 1.0   # rad/s
ESCAPE_TIMEOUT       = 15.0  # sec
ESCAPE_TOLERANCE     = 8.0   # deg
ESCAPE_ROTATION_SAFE = 310   # mm: 회전 방향 측면 장애물 안전 거리 (+10mm)
ESCAPE_EXTRA_ANGLE   = 15    # deg: 목표 각도에 추가하는 여유 회전량
BACKUP_SPEED         = 0.10  # m/s
BACKUP_DURATION      = 0.6   # sec: 후진 시간 (60mm)
...
def execute_escape_rotation(arduino, lidar, all_scan_points):
    ...
    while time.time() - t_start < ESCAPE_TIMEOUT:
        ...
        while lidar.in_waiting >= 5:
            ...
            if result:
                angle_raw, dist, _ = result
                if dist > 0:
                    lidar_buf.append((normalize_angle(angle_raw), dist))
