...
ESCAPE_ROTATION_SAFE = 300   # mm: 회전 방향 측면 장애물 안전 거리
BACKUP_SPEED         = 0.10  # m/s: 탈출 전 후진 속도
BACKUP_DURATION      = 0.5   # sec: 후진 시간 (약 50mm 후진)
...
def execute_escape_rotation(arduino, all_scan_points):
    ...
    1. find_escape_angle()로 목표 방향 계산
    2. 회전 방향 측면 장애물 확인 → 막혀있으면 반대 방향
    3. 후진으로 공간 확보 (BACKUP_SPEED × BACKUP_DURATION)
    4. ESC 전송 (헤딩 리셋 + 헤딩가드 비활성)
    5. v=0, w=±ESCAPE_W 전송
    6. 아두이노 헤딩 피드백으로 목표 도달 확인 후 정지
    """
    global arduino_heading_deg
    ...
    # ── 2. 회전 방향 측면 장애물 확인 ────────────────────────────────────────
    # 오른쪽 회전(w<0) → 오른쪽 반구 확인, 왼쪽 회전(w>0) → 왼쪽 반구 확인
    scan_dict = {}
    for angle_norm, dist in all_scan_points:
        if dist <= 0:
            continue
    ...
    def rotation_side_blocked(sign):
        if sign < 0:   # 오른쪽 회전 → 오른쪽(음수) 반구
    ...
    if rotation_side_blocked(w_sign):
        print(f"  [방향조정] {'오른쪽' if w_sign<0 else '왼쪽'} 막힘 → 반대 방향으로 전환")
        w_sign = -w_sign
    ...
    # ── 3. 후진으로 공간 확보 ─────────────────────────────────────────────────
    print(f"  [후진] {BACKUP_SPEED}m/s × {BACKUP_DURATION}초 (약 {BACKUP_SPEED*BACKUP_DURATION*1000:.0f}mm)")
    ...
    while time.time() - t_start < ESCAPE_TIMEOUT:
        ...
        if abs(arduino_heading_deg) >= abs(target_deg) - ESCAPE_TOLERANCE:
