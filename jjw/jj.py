...
[개선 사항]
  ...
  - stop_zone_w_sign 분리: stop zone / danger zone 방향 메모리 독립
    → stop zone 정면 노이즈 진동 방지가 danger zone 상태와 무관하게 동작
  - stop zone에서 측면 감지 비활성화
    → SIDE_FWD_DEADZONE(130mm) < STOP_FWD_RANGE(180mm) 겹침 구간 충돌 방지
    → stop zone에서는 장애물 각도 즉결이 더 신뢰할 수 있음
"""
...
SIDE_FWD_DEADZONE = 130  # mm: 이 수직거리 이내는 정면으로 간주 → 측면 판정 제외
                          #     stop zone(180mm)보다 작아도 stop zone에서 측면감지
                          #     자체를 비활성화하므로 충돌 없음
...
def get_gap_width(scan_points, ref_angle, ref_dist, is_left):
    ...
    [수정 1] ref_angle 자체 제외 (< / > 사용)
      장애물 자체 포인트부터 탐색을 시작하면
      첫 번째 포인트에서 바로 depth jump가 끊기는 오류 유발

    [수정 2] depth jump 절댓값 처리
      거리 증가(빈 공간) + 거리 감소(더 가까운 벽) 모두 감지

    [수정 3] fallback 보수적 추정
      연속 벽일 때 DETECTION_RANGE 대신 edge_p 거리로 추정
      → 너비 과대 추정 방지
    """
    if is_left:
...
def find_vw_command(scan_points, heading_deg):
    ...
    [실행 순서]
      ...
      2. 선속도(v) 결정
      3. stop zone / danger zone 분기
         ├─ stop zone : 측면 감지 비활성 + 각도 즉결 + stop_zone_w_sign 노이즈 억제
         └─ danger zone: 측면 감지 활성 + get_gap_width → select_direction_by_width
      4. 각속도(w) 계산

    [변수 역할 분리]
      avoidance_w_sign : danger zone 방향 메모리
      stop_zone_w_sign : stop zone 전용 방향 메모리
      → 두 zone이 서로의 방향 상태를 오염시키지 않음
    """
    ...
    if stop_points:
        ...
        # ── Danger zone ────────────────────────────────────────────────────
        # stop zone 탈출 시 stop_zone_w_sign 리셋
        stop_zone_w_sign = 0.0
