...
FGM_RATIO_THRES  = 1.5   # 인접 포인트 거리 비율 이상이면 갭 경계로 인식

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# Cartesian 갭 통과 필터 파라미터
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 갭 center_angle 방향으로 로봇 폭 사각형이 실제 통과 가능한지 검사.
# 수직 거리(점-직선)가 clearance 미만인 포인트가 하나라도 있으면 가짜 갭으로 제거.

GAP_CHECK_SAFETY_MARGIN = 15    # mm: ROBOT_HALF_WIDTH 위에 추가 안전 마진
                                #     → clearance = 110 + 15 = 125mm
GAP_CHECK_MIN_PROJ      = 30    # mm: 궤적 방향 최소 검사 거리 (발밑·후방 노이즈 제외)
GAP_CHECK_MAX_PROJ      = 800   # mm: 궤적 방향 최대 검사 거리 (너무 먼 포인트 제외)
...
SCAN_WIDE_HALF = 135   # 측면 반발력 감지 범위 (각도)
SEND_INTERVAL  = 0.1
...
SIDE_W_BOOST_GAIN    = 3.0   # rad/s: 측방 레이어 w 크기 기여 계수
...
def find_all_gaps(scan_points):
    ...
    모든 갭(장애물 경계 쌍)을 추출. [Doc3 방식 채택]

    너비 계산: 코사인 거리(에지-에지 chord) 대신,
    각 에지에서 반대편 클러스터의 최근접 점까지 유클리드 거리로 측정.
    양방향 중 더 좁은 값을 실제 통과 가능 너비로 사용.
    → 서로 다른 깊이의 물체 사이 갭을 과대 산출하는 문제 해소.
...
def is_gap_passable(scan_points, center_angle_deg):
    """
    center_angle_deg 방향 가상 궤적(로봇 진행선)에 대해
    모든 유효 라이다 포인트의 수직 거리(점-직선 거리)를 계산. [Doc2 방식 채택]

    수직 거리가 ROBOT_HALF_WIDTH + GAP_CHECK_SAFETY_MARGIN 미만인
    포인트가 하나라도 존재하면 False(통과 불가).

    좌표계: x = dist*sin(angle_deg)  (좌 음수, 우 양수)
            y = dist*cos(angle_deg)  (전방 양수)
    궤적 방향 단위벡터: d = (sin θ, cos θ)
    점 P의 수직 거리:   |px·cos θ − py·sin θ|   (외적 크기)
    궤적 방향 투영:     px·sin θ + py·cos θ      (내적, 전방이 양수)
    """
    theta     = math.radians(center_angle_deg)
    sin_t     = math.sin(theta)
    cos_t     = math.cos(theta)
    clearance = ROBOT_HALF_WIDTH + GAP_CHECK_SAFETY_MARGIN

    for angle_norm, dist in scan_points:
        if dist < LIDAR_MIN_VALID or dist > GAP_CHECK_MAX_PROJ:
            continue

        px = dist * math.sin(math.radians(angle_norm))
        py = dist * math.cos(math.radians(angle_norm))

        # 궤적 방향 투영 — 전방 GAP_CHECK_MIN_PROJ~MAX_PROJ 구간만 검사
        proj = px * sin_t + py * cos_t
        if proj < GAP_CHECK_MIN_PROJ:
            continue   # 로봇 발밑·후방 포인트 제외

        # 수직 거리 (외적)
        perp = abs(px * cos_t - py * sin_t)

        if perp < clearance:
            if DEBUG_STOP:
                print(f"    [GAP_FILTER] blocked: angle={angle_norm:+.1f}° "
                      f"dist={dist:.0f}mm  perp={perp:.0f}mm < {clearance:.0f}mm")
            return False

    return True


def choose_escape_gap(gaps, scan_points, prefer_angle=0.0):
    """
    통과 가능한 갭 조건 (3단계 필터):
      1. 폭  >= STOP_ESCAPE_MIN_GAP          [Doc3 방식]
      2. 깊이 >= FGM_MIN_DEPTH_MM            [Doc3 방식]
      3. Cartesian 충돌 필터: 궤적 방향으로  [Doc2 방식]
         로봇 폭 사각형이 실제 통과 가능한지 검사

    조건을 모두 만족하는 갭 중 prefer_angle에 가장 가까운 것을 반환.
    없으면 None (불통과 갭 진입 방지).
    """
    passable = []
    for g in gaps:
        # 1단계: 폭 검사 (클러스터 기반 유클리드 너비)
        if g['width'] < STOP_ESCAPE_MIN_GAP:
            continue
        # 2단계: 깊이 검사
        if g['depth'] < FGM_MIN_DEPTH_MM:
            continue
        # 3단계: Cartesian 충돌 필터
        if not is_gap_passable(scan_points, g['center_angle']):
            if DEBUG_STOP:
                print(f"    [GAP_FILTER] gap@{g['center_angle']:+.1f}° "
                      f"(w={g['width']:.0f}mm) rejected by Cartesian filter")
            continue
        passable.append(g)
...
def find_stop_escape_direction(scan_points, heading_deg=0.0):
    ...
    chosen = choose_escape_gap(gaps, scan_points, prefer_angle=heading_deg)
    ...
    gap_info = [
        {
            ...
            'depth':        g['depth'],
            # passable 판정: 3단계 필터 모두 반영
            'passable':     (g['width'] >= STOP_ESCAPE_MIN_GAP
                             and g['depth'] >= FGM_MIN_DEPTH_MM
                             and is_gap_passable(scan_points, g['center_angle'])),
            'chosen':       g is chosen,
        }
        for g in gaps
    ]
...
def get_side_repulsion(scan_points):
    ...
    """
    side_inner = ROBOT_HALF_WIDTH                    # 110mm
    side_outer = ROBOT_HALF_WIDTH + SIDE_SAFE_MARGIN # 300mm
...
def main():
    ...
    print(f"  GAP width   : cluster nearest-Euclidean (Doc3) — cross-depth overestimate fixed")
    print(f"  GAP filter  : Cartesian trajectory check (Doc2), "
          f"clearance={ROBOT_HALF_WIDTH + GAP_CHECK_SAFETY_MARGIN}mm "
          f"(half_w={ROBOT_HALF_WIDTH} + margin={GAP_CHECK_SAFETY_MARGIN}), "
          f"proj=[{GAP_CHECK_MIN_PROJ}, {GAP_CHECK_MAX_PROJ}]mm")
