...
def get_narrow_gap_pushes(scan_points, layer, in_stop=False):
    ...
    for ao, do in opening_edges:
        ...
        deficit_ratio = (MIN_PASSAGE_WIDTH - gap_width) / MIN_PASSAGE_WIDTH
        strength      = deficit_ratio * layer['horiz_th'] * VIRTUAL_OBS_GAIN * overlap_scale
        ...
        if abs(center_angle) < VIRTUAL_CENTER_DEADBAND:
            ...
            if DEBUG_VIRTUAL:
                print(f"  [VIRTUAL/{layer['name']}] CENTER "
                      f"gap={gap_width:.0f}mm center={center_angle:+.1f}° "
                      f"scale={overlap_scale:.1f} → both={half:.0f}mm (deadband)")
        elif center_angle < 0:
            virtual_push_left  = max(virtual_push_left,  strength)
            if DEBUG_VIRTUAL:
                print(f"  [VIRTUAL/{layer['name']}] L "
                      f"gap={gap_width:.0f}mm center={center_angle:+.1f}° "
                      f"scale={overlap_scale:.1f} → vL={strength:.0f}mm")
        else:
            virtual_push_right = max(virtual_push_right, strength)
            if DEBUG_VIRTUAL:
                print(f"  [VIRTUAL/{layer['name']}] R "
                      f"gap={gap_width:.0f}mm center={center_angle:+.1f}° "
                      f"scale={overlap_scale:.1f} → vR={strength:.0f}mm")
...
def main():
    ...
    print(f"  Virtual obs : MIN_PASSAGE={MIN_PASSAGE_WIDTH}mm "
          f"GAIN={VIRTUAL_OBS_GAIN} DEADBAND=±{VIRTUAL_CENTER_DEADBAND}°")
