...
def is_gap_passable(scan_points, center_angle_deg):
    ...
    모든 유효 라이다 포인트의 수직 거리(점-직선 거리)를 계산.
...
def find_all_gaps(scan_points):
    ...
    for i in range(len(pts) - 1):
        ...
        # 갭 중심: 두 엣지점의 Cartesian 중점 → 각도 변환
...
def choose_escape_gap(gaps, scan_points, prefer_angle=0.0):
    """
    통과 가능한 갭 조건 (3단계 필터):
      1. 폭  >= STOP_ESCAPE_MIN_GAP
      2. 깊이 >= FGM_MIN_DEPTH_MM
      3. Cartesian 필터: 갭 방향 궤적으로 로봇 폭 사각형이 실제 통과 가능
...
def main():
    ...
    print(f"  GAP filter  : Cartesian check, clearance={ROBOT_HALF_WIDTH + GAP_CHECK_SAFETY_MARGIN}mm "
