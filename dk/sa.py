...
# LAB 색 검출(camera)이 색지/장애물을 직접 분리하므로 floor_verify 미사용.
...
LAYERS = [
    # L1: 가장 가까움, 동적 가중치, weight_cap=7.5, v_max=0.2
...
SEND_INTERVAL  = 0.1
SCAN_STALE_SEC = 0.5   # sec: 라이다가 이 시간 이상 새 스캔을 못 내면 stale → 안전 정지
                       # (RPLIDAR 1회전 ~0.1~0.2s 기준 여유. 라이다 멈춤 워치독)

# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 측면 반발력 파라미터 (horiz 190mm × fwd 160mm 감지 구간)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
SIDE_SAFE_MARGIN  = 300   # mm: 로봇 측면 안전 마진 (side_outer = 110+300 = 410mm)
...
# [히스테리시스] 경계폭(260mm) 근처에서 보너스↔척력이 매 프레임 뒤집혀
#   "들어갈까 말까"를 반복하는 진동을 막는다.
#   - 보너스(끌림)는 PASSABLE_WIDTH_HI 이상일 때만 → 확실히 넓은 갭만 추종
#   - 척력(밀어냄)은 MIN_PASSAGE_WIDTH 미만일 때만 (기존 유지)
#   - 그 사이(260~PASSABLE_WIDTH_HI) 회색지대 → 보너스는 없고 약한 척력만:
#     끼임은 막되, 경계폭 노이즈로 판정이 튀어도 거동이 매끄럽게 이어짐
PASSABLE_WIDTH_HI       = STOP_ESCAPE_MIN_GAP + 40   # 300mm: 보너스 받을 최소 폭
...
# (가상 경계 제거됨)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# 기존 "색 미감지 시 현재 위치 중심 1.5m 가상 원" 방식은
# 색을 잃을 때마다 중심이 갱신되어 경계 역할을 못 하는 문제가 있었음.
# → search_planner의 고정 격자(_origin 최초 1회 고정) 커버리지로 대체.

# ── CLOSE 진입 게이트 ──────────────────────────────────────────────
# 이전엔 floor_verify(라이다 거리비교)로 색지 검증 후 CLOSE를 허용했으나,
# 이제 LAB 검출이 황토/남색 장애물을 자체 분리한다. 다만 카메라 한 프레임만
# 색이 보여도 CLOSE로 빨려들면, LAB 노이즈성 깜빡임(좁은 tol 등)에 잘못
# 진입할 수 있다. 그래서 '연속 N프레임 검출'을 요구해 게이트를 강화한다.
CLOSE_VERIFY_WINDOW   = 1.0   # sec: 마지막 확정 검출 이후 CLOSE 진입 허용 윈도우
CLOSE_CONFIRM_FRAMES  = 3     # 이 횟수만큼 연속 검출돼야 CLOSE 게이트 통과(노이즈 차단)
DEBUG_VERIFY          = 0     # (구버전) 검증 판정 로그 — LAB 전환 후 사용 안 함
...
_arduino_rx_buf       = bytearray()   # 아두이노 수신 바이트 버퍼 (논블로킹 라인 조립용)
prev_w                = 0.0
...
# ★ 가상 경계 전역 상태
_last_paper_verify_time = 0.0  # 마지막으로 '색지 검증 통과'한 시각 (CLOSE 게이트)
_color_confirm_count    = 0    # 연속 색 검출 프레임 수 (CLOSE_CONFIRM_FRAMES 게이트)

# ── 스레드 공유 상태 ─────────────────────────────────────────────────────────
_scan_lock        = threading.Lock()
_latest_scan      = []            # 라이다 스레드가 완성된 스캔을 여기에 기록
_latest_scan_time = 0.0           # 마지막으로 완성 스캔을 기록한 시각 (staleness 워치독용)
...
def read_arduino(arduino):
    """아두이노 오도메트리 수신 (논블로킹).
    in_waiting 만큼만 읽어 버퍼에 모으고, '\\n'으로 끝난 완성 줄만 파싱한다.
    미완성 잔여는 버퍼에 남겨 다음 호출에서 이어 붙인다 — readline()이 줄
    중간에서 timeout(1s)까지 블로킹해 제어 루프를 멈추는 문제를 없앤다."""
    global arduino_heading_deg, arduino_x_mm, arduino_y_mm, _arduino_rx_buf
    n = arduino.in_waiting
    if n:
        _arduino_rx_buf.extend(arduino.read(n))   # 있는 만큼만 (블로킹 없음)
    if len(_arduino_rx_buf) > 4096:               # 줄바꿈 없는 폭주 방지: 최근 일부만 유지
        del _arduino_rx_buf[:-256]
    while True:
        idx = _arduino_rx_buf.find(b'\n')
        if idx < 0:
            break
        line = _arduino_rx_buf[:idx].decode('utf-8', errors='ignore').strip()
        del _arduino_rx_buf[:idx + 1]
        try:
...
def _compute_close_target():
    ...
    bearing_global_deg = arduino_heading_deg + camera.get_last_close_bearing()
    dist_mm            = camera.get_estimated_distance_mm()
...
def find_stop_escape_direction(scan_points, heading_deg=0.0):
    ...
    # 갭 center_angle은 상대각(라이다 프레임, +=오른쪽). jw_won.py 부호규약에서
    # 갭의 글로벌 방향 = heading_deg - center_angle 이므로(라이다는 heading과
    # 반대 부호), 글로벌 0°(원래 정면)에 가장 가까운 갭은 center_angle 이
    # heading_deg 에 가장 가까운 갭이다.
...
def get_side_repulsion(scan_points):
    ...
    side_outer = ROBOT_HALF_WIDTH + SIDE_SAFE_MARGIN  # 감지 끝: 110 + 300 = 410mm
...
def get_front_passable_gaps(scan_points):
    ...
    갭 통과 조건:
      ...
      - depth  >= FRONT_GAP_MIN_DEPTH  (갭 중심 방향 실제 스캔 거리, 없으면 FGM_MAX_RANGE_MM)
    점수: width × depth
    ...
    def depth_at_center(center_ang):
        """갭 중심 각도에 가장 가까운 스캔 포인트 거리 (10° 이내 없으면 FGM_MAX_RANGE_MM)."""
        if not front:
    ...
    for i in edge_indices:
        ...
        # [히스테리시스] 보너스는 확실히 넓은 갭(>= HI)만 — 경계폭 진동 방지
        if width < PASSABLE_WIDTH_HI:
            continue
...
def get_narrow_gap_pushes(scan_points, layer, in_stop=False):
    ...
    for ao, do in opening_edges:
        ...
        # [히스테리시스] 척력은 HI 미만 갭에 건다.
        #   260~300mm 회색지대는 약한 척력으로 — 보너스도 없고 척력도 0이면
        #   로봇이 좁은 갭으로 그냥 직진해 끼는 것을 막는다.
        if gap_width >= PASSABLE_WIDTH_HI:
            continue
        ...
        # ── 지수함수 척력 계산 ────────────────────────────────────────────
        # t: 갭 여유 비율 (0=완전 막힘, 1=확실히 통과 가능 폭 HI에 도달)
        #   분모를 PASSABLE_WIDTH_HI로 둬 260~300mm 회색지대에서도
        #   t<1 → 약하지만 0이 아닌 척력이 형성된다.
        t         = gap_width / PASSABLE_WIDTH_HI
...
def find_vw_layered(scan_points, heading_deg, target_bearing=0.0):
    ...
    [좌표계] target_bearing 은 라이다 프레임(+=오른쪽)으로 통일해 받는다.
      - scan_points / 갭 center_angle / STOP 피벗 / 플래너 tb 가 모두 +=오른쪽이라
        is_target_blocked·choose_target_gap·조향이 변환 없이 일관된다.
      - 카메라 bearing(+=왼쪽)은 호출 전에 부호를 반전(-bearing)해 넘긴다.
      - 라이다 프레임 조향법칙: w = -KP × (각도)  (branch ②/④ 및 STOP 피벗과 동일)
    """
    ...
    if not blocked:
        ...
        w = -KP_GOAL * desired_heading   # 라이다 프레임(+=오른쪽) → -KP (branch ②와 동일)

        # 목표 정렬도에 따라 v 조정
        # 정렬됨(0°) → FORWARD_SPEED / 많이 벗어남(≥TARGET_ALIGN_ANGLE) → MIN_SPEED
        align_factor = max(0.0, 1.0 - abs(desired_heading) / TARGET_ALIGN_ANGLE)
        v_align = MIN_SPEED + (FORWARD_SPEED - MIN_SPEED) * align_factor
        # 목표 콘(±TARGET_CLEAR_CONE°) 밖의 근접 장애물도 감속에 반영되도록
        # 레이어 기반 v(step 2 계산)와 정렬 기반 v 중 더 느린 쪽을 택한다.
        v = min(v, v_align)
        ...
        prev_desired_heading = desired_heading
        w = -KP_GOAL * desired_heading  # LiDAR center_angle(+=오른쪽) → -KP
                                        #   (target_bearing 도 라이다 프레임으로 통일돼
                                        #    분기①·④와 같은 부호규약을 따른다)
        if DEBUG_GAP:
        ...
        desired_heading      = target_bearing
        prev_desired_heading = desired_heading
        w = -KP_GOAL * target_bearing   # 라이다 프레임(+=오른쪽) → -KP
        if DEBUG_CLEAR:
...
def _stop_set_pivot(heading_deg, target, gap_width):
    ...
    if gap_width == 0:
        stop_locked_global_heading = 0.0
        # 진입 사이클도 phase 2와 같은 STOP_PIVOT_MAX_W 크기로 (1.8→0.9 불연속 제거).
        # phase 2는 stop_pivot_w의 부호만 쓰므로 크기를 낮춰도 회전 방향은 동일.
        ...
        #   +각도=오른쪽(라이다),  +w=CCW(좌회전).
        # 갭이 오른쪽(target>0)이면 글로벌 목표 헤딩은 heading-target 이고,
        # 그쪽(오른쪽)으로 돌려면 -w(CW) 여야 한다.
        # (회전 방향과 감속 목표 헤딩은 한 쌍으로 같은 부호규약을 따른다)
...
def _lidar_reader(lidar):
    """라이다 수신 전용 스레드.
    lidar.read() 블로킹이 모터 루프에 영향을 주지 않도록 분리.
    한 바퀴 완성 시 중복 각도 제거 후 _latest_scan에 덮어쓰기 (누적 없음).

    [재동기화] read()가 타임아웃으로 5바이트 미만을 반환하면 패킷 경계가
    어긋난다. 바이트를 버퍼에 모아 두고, 유효 프레임이 나올 때까지 1바이트씩
    버려가며(parse_packet의 플래그 검증 이용) 프레임 정렬을 복구한다."""
    global _latest_scan_time
    local_pts = []
    buf = bytearray()
    while not _shutdown.is_set():
        try:
            chunk = lidar.read(5)   # 최대 5바이트 (타임아웃 시 더 적게 올 수 있음)
        except Exception:
            continue
        if not chunk:
            continue
        ...
        # 패킷 경계 재동기화: 유효 프레임이 나올 때까지 1바이트씩 버린다.
        while len(buf) >= 5:
            frame  = bytes(buf[:5])
            result = parse_packet(frame)
            if result is None:
                del buf[0]          # 정렬 어긋남 → 1바이트 버리고 재시도
                continue
            del buf[:5]             # 유효 프레임 소비
            ...
            if s_flag == 1 and local_pts:
                ...
                with _scan_lock:
                    ...
                    _latest_scan_time = time.time()   # staleness 워치독용 갱신 시각
                local_pts = []
            local_pts.append((
...
def _motor_controller(arduino):
    ...
    global prev_w, _close_target_x, _close_target_y, _close_initial_dist, _close_observe_start, \
           _last_paper_verify_time, _color_confirm_count
    last_cmd_str = ""
    while not _shutdown.is_set():
        read_arduino(arduino)
        with _scan_lock:
            pts = [(a, d) for a, d in _latest_scan if d > 0]
            scan_age = time.time() - _latest_scan_time
        # 라이다가 멈춰 stale 스캔이 남아 있으면(워치독) 빈 스캔과 동일하게 안전 정지.
        # → 옛 데이터로 장애물 없는 줄 알고 전진하는 것을 막는다.
        if pts and scan_age <= SCAN_STALE_SEC:
            # ── 상태 1: DWELL / DONE → 정지 ────────────────────────────────
            if camera.is_done() or camera.is_dwelling():
                v, w = 0.0, 0.0
                prev_w = 0.0
            ...
            # CLOSE 신규 진입은 '최근 카메라 확정 검출'(LAB) 윈도우 안에서만 —
            # 카메라가 잠시 색을 놓친 상태에서 잘못된 CLOSE로 빨려들지 않게 한다.
            # (LAB가 황토/남색 장애물을 자체 분리하므로 별도의 라이다 검증은 불필요.)
            # 관측이 이미 시작됐거나(_close_observe_start) 목표 좌표가 확정됐으면
            # (_close_target_x) 윈도우와 무관하게 CLOSE를 유지한다 — 정지 관측 중
            # is_close()가 흔들려 SEEK로 빠졌다 관측이 재시작되는 핑퐁을 막는다.
            elif ((camera.is_close()
                   and (time.time() - _last_paper_verify_time) < CLOSE_VERIFY_WINDOW)
                  or _close_observe_start is not None
                  or _close_target_x is not None):
                # ── 2a: 관측 단계 (CLOSE_OBSERVE_SEC 동안 정지) ─────────────
                if _close_target_x is None:
                    if _close_observe_start is None:
                        _close_observe_start = time.time()
                        _last_paper_verify_time = time.time()  # 관측 내내 게이트 유지
                ...
                if dist_err < CLOSE_ARRIVE_MM:
                    ...
                    v, w = 0.0, 0.0
                    prev_w = 0.0
                    camera.signal_arrival()   # 카메라 peaked→drop이 막혀도 미션 진행
                    if DEBUG_CLOSE_DONE:
                    ...
                    # 헤딩 오차 크면 옆으로 휘지 않게 감속 + 도착 직전(거리) 감속으로 오버슈트 억제.
                    # 단, MIN_SPEED 바닥을 둬 멈춰서 못 가는 일은 막는다.
                    align_close = max(0.0, 1.0 - abs(hdg_err) / TARGET_ALIGN_ANGLE)
                    near_scale  = min(1.0, dist_err / (CLOSE_ARRIVE_MM * 4.0))
                    v = max(MIN_SPEED, CLOSE_SPEED_MAX * align_close * near_scale)
            ...
            # ── 상태 3: SEEK → 카메라 bearing + 라이다 회피 ────────────────
                ...
                if _close_target_x is not None:
                    print(f"[STATE] CLOSE → SEEK  "
                          f"is_close={camera.is_close()}  "
                          f"is_done={camera.is_done()}  "
                          f"is_dwelling={camera.is_dwelling()}")
                _close_target_x = _close_target_y = _close_initial_dist = _close_observe_start = None
                bearing = camera.get_bearing()

                # ── 3a: 확정 검출 → 직접 추종 ───────────────────────────
                # LAB 검출이 황토/남색 장애물을 자체 분리하므로 라이다 검증은 생략.
                # 단, 한 프레임 깜빡임(LAB 노이즈)에 CLOSE로 빨려들지 않도록
                # 연속 CLOSE_CONFIRM_FRAMES 프레임 검출돼야 게이트를 갱신한다.
                #   - 색이 보이는 프레임  → 카운터 +1, 추종은 즉시 시작(추종은 안전)
                #   - 카운터 ≥ 임계       → CLOSE 게이트 시각 갱신(이때만 CLOSE 진입 허용)
                #   - 색을 놓치면         → 카운터 0으로 리셋
                verified = (bearing is not None)
                if verified:
                    _color_confirm_count += 1
                    search_planner.notify_color_visible(
                        arduino_x_mm, arduino_y_mm)
                    # 연속 검출이 임계 이상일 때만 CLOSE 진입 게이트를 연다.
                    if _color_confirm_count >= CLOSE_CONFIRM_FRAMES:
                        _last_paper_verify_time = time.time()
                    # 카메라 bearing(+=왼쪽) → 라이다 프레임(+=오른쪽)으로 부호 반전
                    v, w = find_vw_command(pts, arduino_heading_deg,
                                           target_bearing=-bearing)
                else:
                    _color_confirm_count = 0   # 색 놓침 → 연속 카운터 리셋
            ...
            # STOP 피벗은 자체 감속 로직이 있어 W_SMOOTH 평활화를 우회한다
            # (직전 반대방향 w와 30% 섞여 탈출 회전이 굼떠지는 것 방지).
            if stop_phase != 0:
                prev_w = w
            ...
            # 빈 스캔(라이다 한 프레임 누락/전 점 무효) 또는 stale 스캔
            # (라이다가 멈춰 SCAN_STALE_SEC 이상 갱신 없음) → 안전 정지.
            # else가 없으면 이전 v,w 명령이 아두이노에 그대로 유지되어,
            # 전진 중이었다면 라이다가 안 보이는 사이 장애물로 돌진한다.
            prev_w = 0.0
            _color_confirm_count = 0   # 스캔 끊김 동안 연속 검출 카운터도 리셋
            cmd = "0.00 0.00\n"
            arduino.write(cmd.encode())
            if cmd != last_cmd_str:
                if DEBUG_SEND:
                    print("[SEND] 빈/stale 스캔 → 안전 정지 (v=0, w=0)")
                last_cmd_str = cmd
        time.sleep(SEND_INTERVAL)
...
def main():
    ...
    time.sleep(1)
    lidar.reset_input_buffer()          # RESET 후 부팅 배너 비우기 → 스캔 디스크립터 정렬 보장
    lidar.write(bytes([0xA5, 0x20]))   # SCAN
    lidar.read(7)                       # 스캔 응답 디스크립터(7B) 소비
