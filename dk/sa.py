...
def find_vw_layered(scan_points, heading_deg, target_bearing=0.0):
    ...
    if not blocked:
        ...
        prev_desired_heading = desired_heading
        w = -KP_GOAL * desired_heading  # ★ LiDAR center_angle(+=오른쪽) ↔ 조향 부호 반대 → -KP
                                        #   (분기① 카메라 bearing(+=왼쪽, +KP)과 다름.
                                        #    jw_won.py / camera_tracker.py 부호규약과 동일)
        if DEBUG_GAP:
...
def _motor_controller(arduino):
    ...
    while not _shutdown.is_set():
        ...
        if pts:
            ...
            # 빈 스캔(라이다 한 프레임 누락/전 점 무효) → 안전 정지.
