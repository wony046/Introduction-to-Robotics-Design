...
# 이제 LAB 검출이 황토/남색 장애물을 자체 분리하므로, 카메라 확정 검출이
# 있는 동안엔 검증 게이트를 매 주기 통과시킨다 (윈도우 의미만 유지).
CLOSE_VERIFY_WINDOW = 1.0      # sec: 마지막 카메라 확정 검출 이후 CLOSE 진입 허용 윈도우
