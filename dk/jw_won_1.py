...
APPROACH_LOCK_MM    = 600.0    # mm: 검증 통과 + 이 거리 안 → 접근 잠금(재검증 생략)
...
_approach_lock = False         # 검증 통과 후 접근 시작 → 이후 재검증 생략(CLOSE까지 유지)
...
def _motor_controller(arduino):
    ...
    global prev_w, _close_target_x, _close_target_y, _close_initial_dist, _close_observe_start, \
           _last_paper_verify_time, _approach_lock
    last_cmd_str = ""
    while not _shutdown.is_set():
        ...
        if pts:
            ...
            if camera_tracker.is_done() or camera_tracker.is_dwelling():
                v, w = 0.0, 0.0
                prev_w = 0.0
                _approach_lock = False   # 다음 색은 다시 검증부터
                _close_target_x = _close_target_y = _close_initial_dist = _close_observe_start = None

            # ── 상태 2: CLOSE → 정지 관측 후 오도메트리 위치 제어 ──────────
            # _close_target_x가 이미 세팅됐거나 관측이 시작됐으면(_close_observe_start)
            # 카메라가 한 프레임 색지를 놓쳐 is_close()가 깜빡여도 CLOSE를 유지한다.
            # (관측 중 SEEK로 떨어지면 _close_observe_start가 지워져 관측이
            #  처음부터 재시작 → 0.2s까지 갔다 1.0s로 리셋되는 버그를 막는다.)
            elif ((camera_tracker.is_close()
                   and (_approach_lock
                        or (time.time() - _last_paper_verify_time) < CLOSE_VERIFY_WINDOW))
            ...
            # ── 상태 3: SEEK → 카메라 bearing + 라이다 회피 ────────────────
                ...
                # ── 3a: 확정 검출 → 라이다 검증(원거리에서만) 후 추종 ──────
                # 접근 잠금: 한 번 검증 통과해 가까이(APPROACH_LOCK_MM) 다가가면
                # 이후엔 재검증을 생략한다. 색지에 근접할수록 카메라는 색지가
                # 화면 아래로 내려가 약해지고 라이다는 주변을 가까이 보면서
                # 검증이 불안정해지는데, 매 프레임 재검증하면 통과↔실패가
                # 깜빡여 'CLOSE 진입 실패 → 멈칫하고 다시 출발'이 된다.
                verified = False
                if bearing is not None:
                    cam_d = camera_tracker.get_estimated_distance_mm()
                    if _approach_lock or camera_tracker.is_close():
                        # 이미 접근/근접 단계 → 검증 묻지 않고 통과 처리
                        verified = True
                    else:
                    ...
                    if verified:
                        ...
                        if cam_d < APPROACH_LOCK_MM:
                            _approach_lock = True              # 잠금 ON
                ...
                if bearing is None or not verified:
                    _approach_lock = False   # 색을 놓치면 잠금 해제
