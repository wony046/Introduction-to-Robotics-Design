...
# ── 색지/장애물 구분 검증 (floor_verify) ─────────────────────────────
# 황토색/남색 장애물이 노랑/파랑으로 오인되는 것을 라이다 거리 비교로 차단.
# CLOSE 진입은 "최근 검증 통과" 이력이 있을 때만 허용한다.
CLOSE_VERIFY_WINDOW = 1.0      # sec: 검증 통과 후 이 시간 안에만 CLOSE 진입 허용
DEBUG_VERIFY        = 1        # 검증 판정 로그
...
def _motor_controller(arduino):
    ...
    while not _shutdown.is_set():
        ...
        if pts:
            ...
            # CLOSE 신규 진입은 '최근 색지 검증 통과'(floor_verify)가 있을 때만 —
            # 장애물(황토/남색) 오인 blob에 근접해도 CLOSE로 빨려들지 않게 한다.
            # 관측이 이미 시작됐거나(_close_observe_start) 목표 좌표가 확정됐으면
            # (_close_target_x) 검증 윈도우와 무관하게 CLOSE를 유지한다.
            # → 정지 관측 중 is_close()/검증윈도우가 흔들려 SEEK로 빠졌다가
            #   관측이 처음부터 재시작되는 핑퐁을 막는다.
            ...
            # ── 상태 3: SEEK → 카메라 bearing + 라이다 회피 ────────────────
                ...
                # ── 3a: 확정 검출 → 라이다로 '바닥 색지' 검증 후 추종 ──────
                verified = False
                if bearing is not None:
                    cam_d = camera_tracker.get_estimated_distance_mm()
                    verified, _ld = floor_verify.verify_detection(
