...
SAFETY_MARGIN    = 10       # 안전 여유 (mm)
# → threshold = 120mm : 수평거리가 이보다 작으면 위험
...
# ※ 긴급 시 v와 w를 같은 비율로 함께 감소 (전반적 감속)
...
def find_vw_command(scan_points):
    ...
    # 긴급 시 v와 w를 함께 줄이기 위한 스케일 계수 (1.0 = 정상, EMERGENCY_MIN_SPEED/FORWARD_SPEED = 최저)
    if in_emergency:
        ratio = n_fwd / EMERGENCY_FWD_RANGE                              # 0.0 ~ 1.0
        min_scale = EMERGENCY_MIN_SPEED / FORWARD_SPEED                  # 0.25
        speed_scale = min_scale + (1.0 - min_scale) * ratio              # 0.25 ~ 1.0
    ...
    # ── 6. 회피각 계산 (수평·수직 이동거리 기반 atan2) ──────────────────────
    #
    #   [기하학적 의미]
    #
    #   obstacle ●──── n_horiz ────┤← threshold →┤
    #            │                               로봇 중심선
    #            n_fwd
    #            │
    #          [robot]
    #
    #   장애물은 이미 n_horiz 만큼 측면에 있음
    #   로봇 끝(= threshold)까지 추가로 필요한 수평 이동량:
    #     delta_horiz = threshold - n_horiz
    #
    #   그 수평 이동을 n_fwd 안에 해내야 하므로:
    #     avoid_angle = atan2(delta_horiz, n_fwd)
    #
    #   예) threshold=120mm, n_fwd=400mm, n_horiz=80mm
    #     delta = 120-80 = 40mm
    #     angle = atan2(40, 400) = 5.7°  ← 실제 필요한 만큼만 꺾음
    #
    #   예) threshold=120mm, n_fwd=400mm, n_horiz=10mm (거의 정면)
    #     delta = 120-10 = 110mm
    #     angle = atan2(110, 400) = 15.4°  ← 정면에 가까울수록 더 많이 꺾음
    #
    delta_horiz = threshold - n_horiz          # 추가로 필요한 수평 이동량 (mm)
    avoid_angle = math.atan2(max(delta_horiz, 1.0), max(n_fwd, 1.0))   # 라디안
    ...
    # 긴급구역: v와 동일한 비율로 w도 감소 (전반적으로 천천히 회피)
    w *= speed_scale
